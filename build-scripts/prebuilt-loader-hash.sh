...
env -i DEVELOPER_DIR=${DEVELOPER_DIR} SRCROOT=${SRCROOT} xcrun -v -sdk macosx clang++ -target arm64-apple-macos13.0 -DBUILDING_DYLD=1 -std=c++2a -w -Wno-incompatible-sysroot -fsyntax-only -Xclang -fdump-record-layouts -Icommon -Idyld -Iinclude -Icache_builder -Ilsl -Imach_o -Iinclude/mach-o -isystem ${SRCROOT}/internal_stubs -isystem ${SRCROOT}/vendor/libplatform/private -idirafter ${SRCROOT}/vendor/xnu/EXTERNAL_HEADERS/corecrypto -idirafter ${SRCROOT}/vendor/xnu/bsd -DDYLD_EXCLAVEKIT_UNAVAILABLE= -DPLATFORM_IOSMAC=6 -DSUPPORT_ARCH_arm64e=1 -DSUPPORT_ARCH_arm64_32=1 ${DERIVED_FILE_DIR}/test.cpp >${DERIVED_FILE_DIR}/test.out
